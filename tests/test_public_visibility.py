...
class PublicVisibilityTest(unittest.TestCase):
    ...
    def test_runs_lists_existing_runs(self) -> None:
        ...
        self.assertIn("effective_phase_providers", listed)
        self.assertEqual(listed["effective_phase_providers"]["write"]["provider"], "reasonix_cli")
        self.assertIn("run_metrics", listed)
        self.assertIn("routing_evidence", listed)
        self.assertIn("efficiency_summary", listed)
        self.assertIn("provider_trail", listed)
        self.assertEqual(listed["run_metrics"]["routing_evidence"], listed["routing_evidence"])
        self.assertEqual(listed["run_metrics"]["efficiency_summary"], listed["efficiency_summary"])
    ...
    def test_background_cancel_failure_text_renders_cancel_result_without_payload_dump(self) -> None:
        from scripts.ai_flow import service

        run_id = "20260604-background-cancel-failure-text"
        run_path = self.repo / ".ai" / "runs" / run_id
        run_path.mkdir(parents=True)
        service._record_job(
            run_path,
            {
                ...
                "pid": -1,
                "run_id": run_id,
                "task": "background cancel failure text view",
                "started_at": "2026-06-04T00:00:00+00:00",
                "started_at_epoch": 0,
                "root": str(self.repo),
                "run_dir": str(run_path),
        ...
        self.assertIn("Patchbay background cancel:", cancel.stdout)
        self.assertIn("ok: False", cancel.stdout)
        self.assertIn("canceled: False", cancel.stdout)
        self.assertIn("error: invalid pid: -1", cancel.stdout)
        self.assertIn("cancel_result: attempted=False, terminated=False, error=invalid pid: -1", cancel.stdout)
        self.assertNotIn("cancel_result: {", cancel.stdout)
    ...
    def test_chinese_readme_surfaces_custom_economy_provider_agent_prompts(self) -> None:
        ...
        self.assertIn("不要询问我", text)
        self.assertIn("别找我", text)
        self.assertIn("别再询问我", text)
        self.assertIn("所有权限给你", text)
        self.assertIn("所有权限全都给你", text)
        self.assertIn("全部权限给你", text)

...
- `patchbay_metrics` when only phase durations, attempts, event/trace counts, provider usage, cost/token availability, and write/fix `routing_evidence` are needed. Inspect `routing_evidence.economy_health.status` for `healthy`, `pending_evidence`, `drift`, or `not_configured` before assuming high-volume write/fix work is using the cheaper Reasonix/DeepSeek route. Prefer the returned top-level `actions[]` or `routing_evidence.actions[]` for UI buttons and follow-ups; they are structured as safe `local_agent` or `diagnostic_tab` actions so hosts do not need to parse `economy_health.next_action`.
...
- `patchbay_config_profile_show` / `patchbay_config_profile_apply` with `profile: "economy"` to inspect or apply the economy route for high-volume write/fix work while leaving plan/review choices intact. These return structured `actions[]`; use them for safe readiness/start/apply follow-ups instead of parsing `next_actions` prose. If `commands.reasonix` is missing, readiness will first surface `configure_reasonix_command` so the cheaper write/fix route can actually execute.
- `patchbay_doctor` for read-only setup diagnostics across config, MCP tools, CLI entry points, and Skill installation. Pass `host` when targeting a specific MCP host; use its `actions[]` for safe setup, Skill, MCP probe, refresh, and economy-profile actions with concrete host-aware commands.

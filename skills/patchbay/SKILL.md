...
- `patchbay_setup` / `patchbay_install` for one-call local setup: project files, local config, Codex Skill installation, MCP registration attempt, doctor summary, top-level `routing` / `routing.workload_policy`, top-level `recommendations`, recommendation-derived `next_actions`, and structured `actions[]` for safe follow-ups.
...
- `patchbay_config_profile_show` / `patchbay_config_profile_apply` with `profile: "economy"` to inspect or apply the economy route for high-volume write/fix work while leaving plan/review choices intact. These return structured `actions[]` plus `routing.workload_policy`, which labels `write`/`fix` as economy phases for simple high-volume implementation and repair, and `plan`/`review` as supervision phases. Use it for UI explanations instead of parsing prose. If `commands.reasonix` is missing, readiness will surface `configure_reasonix_command` and may also expose `configure_deepseek_provider`; prefer sending `configure reasonix command`, `configure reasonix command to <path>`, `配置 Reasonix 命令`, or `把 Reasonix 命令设为 <path>` through `patchbay_agent` for the built-in route, or `configure DeepSeek provider` when the user wants a custom low-cost writer template.

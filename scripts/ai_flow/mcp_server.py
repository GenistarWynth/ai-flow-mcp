...
def patchbay_cancel(run_id: str) -> dict[str, Any]:
    return service.cancel_background_job(ROOT, run_id)
...
def patchbay_doctor(
    ...
    return run_doctor(
        ROOT,
        include_mcp=include_mcp and not skip_mcp,
...
CANONICAL_TOOLS: dict[str, Callable[..., Any]] = {
    ...
    "patchbay_cancel": patchbay_cancel,
...
LEGACY_TOOLS: dict[str, Callable[..., Any]] = {
    ...
    "ai_flow_cancel": patchbay_cancel,
...
def _tool_schema(name: str) -> dict[str, Any]:
    if name.endswith("_agent"):
        properties: dict[str, Any] = {
            "message": {
                "type": "string",
                "description": "Natural-language task or instruction for the conversational Patchbay Agent. Explicit setup/help/status/readiness/routing/gate-status prompts are handled locally without starting a model run; help prompts include `help`, `Patchbay 怎么用`, and `使用说明`, and return `capabilities[]` plus host-specific setup/readiness `actions[]` for Codex, Claude Code, Claude Desktop, and Gemini CLI; when run_id is supplied, help also returns selected-run `gate_diagnosis.next_action`, `next_action`, status/context, and safe diagnostic/setup actions without advancing gates; responses with structured actions may also include `action_groups[]` so hosts can group background polling, background control, economy routing, setup, diagnostics, gated actions, new-task controls, and copyable commands without parsing action ids; readiness prompts include `readiness`, `readiness for Claude Desktop`, `patchbay doctor`, `检查环境`, `环境自检`, and `检查 Gemini 命令行环境`, and host-targeted readiness replies include `setup_host`/`doctor.host`; status/runs prompts include `status`, `查看最近运行`, and `任务列表`; next-step prompts include `what should I do next`, `next step`, `现在该干什么`, and `下一步是什么`, and return `action: next_step` plus `next_action` / `run_reference.next_action` with safe and confirmation metadata without advancing gates; gate-status prompts include `why can't I apply`, `what is blocking apply`, `门禁状态`, and `为什么不能应用`, and return `action: gate_status` plus `gate_diagnosis.next_action` without advancing gates; direct `apply` on a selected run also returns `gate_diagnosis.next_action` and safe diagnostic actions when apply is blocked, instead of requesting confirmation; routing questions such as `what model will write/fix use`, `is writer using cheap model`, or `现在写手是不是走便宜模型` return read-only `action: profile_show`, and include `metrics.efficiency_summary` when a run_id is supplied; setup prompts can name a host such as `patchbay setup for Claude Desktop`, `帮我配置 Patchbay 到 Claude 桌面`, `安装到 Claude 桌面`, `install patchbay for Gemini CLI`, `install Codex Skill`, `register MCP for Claude Desktop`, `安装 Codex Skill`, or `注册 MCP 到 Gemini 命令行`; explicit `patchbay setup without MCP` runs local-only setup, standalone `please don't use MCP`, `no MCP`, `use Chrome Skill`, `use Chrome Skill instead of MCP`, `用你自带的浏览器功能`, `少用这个MCP`, `不要用这个MCP`, `不走 MCP`, `走本地模式`, or `只用本地工具` returns `action: local_mode` with safe local CLI/Skill/readiness actions, and `readiness without MCP` suppresses MCP probe/register follow-ups; `configure DeepSeek provider` returns a safe command action for `patchbay config provider add-cli ... --activate-economy`, while `configure DeepSeek provider to <command>` registers that CLI writer and activates it for write/fix; `configure economy provider command to <path>` or `patchbay config --set-key providers.<id>.command --set-value <path>` repairs the active custom economy provider command without starting a run; `apply economy profile`, mixed prompts such as `简单 writer/fix 用 DeepSeek 省钱` or `降本，让简单 writer/fix 走低价模型`, `configure reasonix command`, `configure reasonix command to <path>`, `配置 Reasonix 命令`, and `把 Reasonix 命令设为 <path>` update local routing configuration; `cancel background job`, `stop background job`, `取消后台任务`, or `停止后台任务` with a run_id stop the active background worker without approving, applying, or advancing gates; metrics/cost/token and view prompts such as `diff`, `logs`, `artifact`, or `查看失败原因` with a run_id inspect that run, and without a run_id inspect the latest run if one exists, returning requested_view plus safe diagnostic_tab actions; gate-changing prompts such as approve/continue/apply without run_id return local guidance instead.",
            },
            "run_id": {
                "type": "string",
        ...
        properties = {
            ...
            "skip_mcp": {
                "type": "boolean",
                "description": "Suppress MCP probe/register follow-up actions for local-only readiness.",
            },
            "skill_path": {
                "type": "string",
                "description": "Optional Codex skills root to inspect.",
        ...
        properties = {
            ...
            "skip_mcp": {
                "type": "boolean",
                "description": "Skip MCP host registration helper for local-only setup and suppress MCP follow-up actions.",
            },
    ...
    descriptions: dict[str, str] = {
        "patchbay_agent": (
            ...
            "is writer using cheap model, cancel background job, 停止后台任务, 门禁状态, 为什么不能应用, 现在写手是不是走便宜模型, patchbay doctor, Patchbay 怎么用, "
            "查看最近运行, 任务列表, 检查环境, 环境自检, 检查 Gemini 命令行环境, economy profile, configure DeepSeek provider, "
            "configure DeepSeek provider to <command>, configure economy provider command to <path>, 简单 writer/fix 用 DeepSeek 省钱, 降本，让简单 writer/fix 走低价模型, "
            "configure reasonix command, configure reasonix command to <path>, 配置 Reasonix 命令, and 把 Reasonix 命令设为 <path> "
            "prompts return local answers with structured actions[] and action_groups[] for safe client follow-ups, including background polling/background control "
            "groups, host-specific setup/readiness shortcuts from help, and help with run_id returns selected-run gate_diagnosis.next_action and next_action "
            "without advancing gates; explicit `patchbay setup without MCP` selects local-only setup, while standalone "
            "`please don't use MCP`, `no MCP`, `use Chrome Skill`, `use Chrome Skill instead of MCP`, `用你自带的浏览器功能`, `少用这个MCP`, `不要用这个MCP`, `不走 MCP`, "
            ...
            "routing; mixed cost prompts such as `简单 writer/fix 用 DeepSeek 省钱` or `降本，让简单 writer/fix 走低价模型` apply the economy profile without starting a run; "
            ...
            "configure_economy_provider_command copyable command actions; selected-run cancel background job / 停止后台任务 prompts stop active background workers "
            "without approving, applying, or advancing gates; metrics/cost/token and view prompts like diff/logs/artifact/查看失败原因 "
        ...
        "patchbay_cancel": "Cancel the active background job for a run, mark JOB.json as canceled, release background locks, and leave apply gates untouched.",
        "patchbay_status": "Return current run status and artifacts, including latest cross-phase event.",
        "patchbay_context": "Return the unified handoff digest for resuming a run across MCP hosts, CLI sessions, and the web workbench, including top-level routing_evidence, efficiency_summary, next_actions, action_groups[], and agent_activity.health_cards/conversation_state.suggestions with safe local/diagnostic actions. Active background jobs lift poll_context/status/events and cancel_background_job into next_actions, background_polling action_groups, and background_control action_groups so clients can refresh or stop progress without advancing gates. Composer-style clients should use action_groups[] to group conversation_state.suggestions into background polling, background control, diagnostics, gate, and new-task sections instead of rendering a flat button list.",
        "patchbay_metrics": "Return only run_metrics efficiency evidence: phase durations, tier_usage economy/supervision/execution rollups, attempts, event/trace counts, provider usage, known cost/token fields, efficiency_summary for verified economy token/cost/time share, routing_evidence.economy_health including command_not_ready, and top-level actions[] plus action_groups[] mirrored from routing_evidence.actions[] for safe local_agent, command, or diagnostic_tab routing follow-ups such as configure_reasonix_command or configure_economy_provider_command; hosts may send configure economy provider command to <path> back through patchbay_agent to repair providers.<id>.command directly.",
        "patchbay_doctor": "Run unified read-only readiness checks for CLI shims, config, bundled Skill source, and whether the installed Codex Skill matches that source; skips stdio MCP probing by default and returns next_actions, recommendations, structured actions[], and action_groups[], with host-aware concrete MCP registration actions, probe actions, and a safe local_agent install_skill action when the Codex Skill is missing or outdated. Skill drift is surfaced through checks.skill.status, installed_matches_source, missing_installed_files, changed_installed_files, and extra_installed_files. Set skip_mcp=true for local-only readiness without MCP probe/register follow-up actions.",
        ...
        "patchbay_config_profile_apply": "Apply a recommended routing profile; economy keeps expensive thinking in plan/review and routes write/fix work to Reasonix/DeepSeek, returning structured actions[] and action_groups[] for safe readiness/start/configure_reasonix_command follow-ups.",

...
def _tool_schema(name: str) -> dict[str, Any]:
    ...
    descriptions: dict[str, str] = {
        ...
        "patchbay_doctor": "Run unified read-only readiness checks for CLI shims, config, bundled Skill source, and Skill installation state; skips stdio MCP probing by default and returns next_actions, recommendations, and structured actions[], with host-aware concrete MCP registration actions, probe actions, and a safe local_agent install_skill action when the Codex Skill is missing. Set skip_mcp=true for local-only readiness without MCP probe/register follow-up actions.",
        "patchbay_setup": "Initialize Patchbay project files, create local config, install the Codex Skill, return MCP registration guidance, include a doctor summary, and expose structured actions[] for safe follow-ups. Set skip_mcp=true for local-only setup without MCP registration or follow-up actions.",
        "patchbay_install": "Alias for patchbay_setup: initialize Patchbay project files, create local config, install the Codex Skill, register MCP when possible, include a doctor summary, and expose structured actions[]. Set skip_mcp=true for local-only setup without MCP registration or follow-up actions.",
        ...
        "patchbay_skill_doctor": "Validate the bundled Patchbay Skill source and whether it is installed in the selected Codex skills root, returning safe install_skill local_agent and refresh_skill_doctor actions when follow-up is needed.",

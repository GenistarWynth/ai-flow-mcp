...
def _tool_schema(name: str) -> dict[str, Any]:
    ...
    descriptions: dict[str, str] = {
        "patchbay_agent": "Primary conversational Patchbay Agent tool. Starts, resumes, advances, applies runs, and returns metrics/cost/token evidence while preserving plan/apply approval gates; explicit setup, help, status, readiness, readiness for Claude Desktop, what should I do next, 下一步是什么, why can't I apply, what is blocking apply, 门禁状态, 为什么不能应用, patchbay doctor, Patchbay 怎么用, 查看最近运行, 任务列表, 检查环境, 环境自检, 检查 Gemini 命令行环境, economy profile, configure reasonix command, configure reasonix command to <path>, 配置 Reasonix 命令, and 把 Reasonix 命令设为 <path> prompts return local answers with structured actions[] for safe client follow-ups; next-step prompts return action next_step without advancing gates, gate-status prompts return action gate_status with gate_diagnosis without advancing gates, host-targeted readiness replies include setup_host/doctor.host, setup prompts can target hosts like Claude Desktop, Claude 桌面, Gemini CLI, Gemini 命令行, install Codex Skill, register MCP for Claude Desktop, 安装 Codex Skill, 注册 MCP 到 Gemini 命令行, or 帮我配置 Patchbay 到 Claude 桌面, run-bound prompts such as 查看失败原因 without run_id return local guidance instead of creating a run, and selected-run view prompts return requested_view plus safe diagnostic_tab actions.",

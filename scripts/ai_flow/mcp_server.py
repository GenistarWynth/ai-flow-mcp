...
def _tool_schema(name: str) -> dict[str, Any]:
    ...
    descriptions: dict[str, str] = {
        ...
        "patchbay_metrics": "Return only run_metrics efficiency evidence: phase durations, tier_usage economy/supervision/execution rollups, attempts, event/trace counts, provider usage, known cost/token fields, routing_evidence.economy_health including command_not_ready, and top-level actions[] mirrored from routing_evidence.actions[] for safe local_agent, command, or diagnostic_tab routing follow-ups such as configure_reasonix_command or configure_economy_provider_command; hosts may send configure economy provider command to <path> back through patchbay_agent to repair providers.<id>.command directly.",

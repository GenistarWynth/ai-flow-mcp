...
def _tool_schema(name: str) -> dict[str, Any]:
    if name.endswith("_agent"):
        properties: dict[str, Any] = {
            "message": {
                "type": "string",
                "description": "Natural-language task or instruction for the conversational Patchbay Agent. Explicit setup/help/status/readiness/routing/gate-status prompts are handled locally without starting a model run; help prompts include `help`, `Patchbay 怎么用`, and `使用说明`; readiness prompts include `readiness`, `readiness for Claude Desktop`, `patchbay doctor`, `检查环境`, `环境自检`, and `检查 Gemini 命令行环境`, and host-targeted readiness replies include `setup_host`/`doctor.host`; status/runs prompts include `status`, `查看最近运行`, and `任务列表`; next-step prompts include `what should I do next`, `next step`, `现在该干什么`, and `下一步是什么`, and return `action: next_step` plus safe handoff actions without advancing gates; gate-status prompts include `why can't I apply`, `what is blocking apply`, `门禁状态`, and `为什么不能应用`, and return `action: gate_status` plus `gate_diagnosis` without advancing gates; routing questions such as `what model will write/fix use`, `is writer using cheap model`, or `现在写手是不是走便宜模型` return read-only `action: profile_show`; setup prompts can name a host such as `patchbay setup for Claude Desktop`, `帮我配置 Patchbay 到 Claude 桌面`, `安装到 Claude 桌面`, `install patchbay for Gemini CLI`, `install Codex Skill`, `register MCP for Claude Desktop`, `安装 Codex Skill`, or `注册 MCP 到 Gemini 命令行`; `configure DeepSeek provider` returns a safe command action for `patchbay config provider add-cli ... --activate-economy`, while `configure DeepSeek provider to <command>` registers that CLI writer and activates it for write/fix; `apply economy profile`, `configure reasonix command`, `configure reasonix command to <path>`, `配置 Reasonix 命令`, and `把 Reasonix 命令设为 <path>` update local routing configuration; metrics/cost/token and view prompts such as `diff`, `logs`, `artifact`, or `查看失败原因` with a run_id inspect that run, and without a run_id inspect the latest run if one exists, returning requested_view plus safe diagnostic_tab actions; gate-changing prompts such as approve/continue/apply without run_id return local guidance instead.",
            },
    ...
    descriptions: dict[str, str] = {
        ...
        "patchbay_metrics": "Return only run_metrics efficiency evidence: phase durations, tier_usage economy/supervision/execution rollups, attempts, event/trace counts, provider usage, known cost/token fields, routing_evidence.economy_health including command_not_ready, and top-level actions[] mirrored from routing_evidence.actions[] for safe local_agent, command, or diagnostic_tab routing follow-ups such as configure_reasonix_command or configure_economy_provider_command.",

...
def _tool_schema(name: str) -> dict[str, Any]:
    if name.endswith("_agent"):
        properties: dict[str, Any] = {
            "message": {
                "type": "string",
                "description": "Natural-language task or instruction for the conversational Patchbay Agent. Explicit setup/help/status/readiness/routing prompts are handled locally without starting a model run; setup prompts can name a host such as `patchbay setup for Claude Desktop`, `安装到 Claude 桌面`, or `install patchbay for Gemini CLI`; `apply economy profile`, `configure reasonix command`, and `configure reasonix command to <path>` update local routing configuration; metrics/cost/token prompts require a run_id and return run efficiency evidence; other run-bound prompts without run_id return local guidance instead.",
            },
    ...
    descriptions: dict[str, str] = {
        "patchbay_agent": "Primary conversational Patchbay Agent tool. Starts, resumes, advances, applies runs, and returns metrics/cost/token evidence while preserving plan/apply approval gates; explicit setup, help, status, readiness, economy profile, configure reasonix command, and configure reasonix command to <path> prompts return local answers with structured actions[] for safe client follow-ups, setup prompts can target hosts like Claude Desktop, Claude 桌面, Gemini CLI, or Gemini 命令行, and run-bound prompts without run_id return local guidance instead of creating a run.",

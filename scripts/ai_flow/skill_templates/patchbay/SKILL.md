...
- `patchbay_setup` / `patchbay_install` for one-call local setup: project files, local config, Codex Skill installation, MCP registration attempt, doctor summary, top-level `routing` / `routing.workload_policy`, and structured `actions[]` for safe follow-ups.

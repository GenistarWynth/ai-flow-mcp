...
If the user explicitly asks not to use MCP, says an MCP host keeps asking for confirmation, asks to use Chrome/browser Skill instead of MCP, or asks for local-only work, do not call `patchbay_*` MCP tools. Use the local CLI/Skill path instead, such as `scripts/patchbay agent message "please don't use MCP" --json`, `scripts/patchbay agent message "use Chrome Skill instead of MCP" --json`, `scripts/patchbay setup --no-mcp --json`, `scripts/patchbay doctor --local-only --json`, `scripts/patchbay agent message "patchbay setup without MCP" --json`, `scripts/patchbay agent message "readiness without MCP" --json`, or `scripts/patchbay agent message "install Codex Skill" --json`.

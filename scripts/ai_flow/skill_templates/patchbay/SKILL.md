...
- View-only Agent prompts such as `context`, `handoff context`, `events`, `poll context`, and `poll events` are read-only diagnostics. With a `run_id`, `context` returns `action: "context"` and an Overview `diagnostic_tab`; `events` returns `action: "events"` and a Trace `diagnostic_tab`. Without a `run_id`, these prompts inspect the latest run when one exists and never advance gates.

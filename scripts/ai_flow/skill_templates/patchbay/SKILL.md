...
- Routing questions such as `what model will write/fix use`, `is writer using cheap model`, or `现在写手是不是走便宜模型` are read-only `profile_show` prompts. Only imperative prompts such as `apply economy profile` or `让大量简单写手工作用便宜模型/DeepSeek 去干` should mutate the routing profile.

...
- **Agent help surface**: `patchbay agent message "help" --json` and `patchbay_agent` help prompts return `capabilities[]` summaries alongside safe `actions[]` and `action_groups[]`, so desktop/MCP/Skill clients can render what the Agent can do without parsing prose.

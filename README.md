...
Implicit local browser preferences such as `use Chrome Skill`, `use browser skill`, `use your built-in browser`, or `用你自带的浏览器功能` also select `local_mode` even when the message does not explicitly mention MCP, so they do not create a model run. In the workbench new-task composer, that response is persisted as local-only mode and immediately refreshes readiness with `skip_mcp=true`.

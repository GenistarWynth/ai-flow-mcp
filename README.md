...
The workbench also persists the selected run, diagnostics drawer open state, and active diagnostics tab across refresh/reopen. If the stored run is no longer present, it falls back to the current inbox focus instead of requesting a stale run.

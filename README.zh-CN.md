...
无人值守 selected-run 批准也接受 `full access`、`approve yourself`、`别找我`、`所有权限全都给你`、`无需向我确认`、`完全访问权限`、`自己允许`、`我根本不在身边` 等自然表达；如果没有选中 `run_id`，这些话仍只返回本地指引，不会创建或推进运行。
...
`use Chrome Skill`、`use browser skill`、`use your built-in browser` 或 `用你自带的浏览器功能` 这类隐式本地浏览器偏好，即使没有直接写 `MCP`，也会进入 `local_mode`，不会误创建模型 run。

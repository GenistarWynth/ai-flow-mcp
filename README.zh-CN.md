...
Web workbench 使用同一套对话式流程，并在诊断抽屉里提供“就绪”页。该页面调用统一 doctor 检查但默认不做 MCP stdio 探测，因此可以在桌面 UI 中看到安装与配置缺口，同时避免打开页面时额外启动子进程。就绪页的 MCP host 选择器会把目标 host 传给 doctor/setup，记住上次选中的 setup/readiness host，并把 `Claude Desktop`、`claude desktop`、`claude-desktop` 这类常见写法统一规范化为具体注册命令。该页面还会显示当前 write/fix 路由画像，并按 `action_groups[]` 把结构化 `actions[]` 分成稳定分区，用来执行安全的 setup、Skill、MCP 探测、刷新、经济路由和 Reasonix 命令配置后续操作。没有选中运行时，composer 上方会先展示紧凑“启动上下文”，汇总本地/no-MCP 模式、就绪状态、write/fix 经济路由和安全 setup/economy 动作，再创建新的计划 run。运行 composer 也会用 `patchbay_context.action_groups[]` 分组 `agent_activity.conversation_state.suggestions`，让后台轮询、后台控制、诊断和门禁动作分成独立控制区，而不是平铺按钮；后台任务卡片也会直接展示打开活动、刷新和取消控制。如果已安装的 Codex Skill 过期，就绪检查卡会直接显示 `status: outdated`、缺失/变更/多余的安装文件，以及安全的更新动作。选中运行的对话线程会先展示紧凑“运行概览”，汇总阶段/状态、门禁进度、经济路由健康和最近 provider 证据，再展示详细事件流。侧栏 run inbox 也会为每个 run 展示阶段、门禁、经济路由和最近 provider 的紧凑快速信号，方便打开详情前先分拣多任务队列。Overview 会展示 `efficiency_summary`、`routing_evidence.economy_health` 与 `agent_activity.health_cards`，包括 `economy_efficiency` 和 `economy_load` 卡片，让桌面端直接看到经济路由是健康、待观测、`command_not_ready`、漂移，还是未配置，并看到简单 write/fix 工作的实际 token/cost/time 占比。Trace/活动页会先把选中消息、运行时间线和 provider trace 渲染成可读卡片，原始 JSON 只保留在折叠明细中供精确排查。Diff/差异页会先按文件汇总变更、增删行和 hunk 数，原始 patch 文本保留在折叠明细中。Log/日志页和 Artifacts/产物页会复用 `failure_recovery`、状态错误、优先产物和当前产物预览，先展示失败摘要、建议下一步、错误线索和产物索引，再保留原始预览。Config/配置页会先摘要阶段路由、provider 命令、测试 allowlist、自定义 provider 和工作流安全开关，再把原始配置 JSON 放在折叠明细里。Providers/提供方页会汇总已配置阶段路由、实际 provider usage、事件轨迹、economy 目标、覆盖率和健康状态，让 write/fix 是否真的走低价模型一眼可查。

Workbench 还会在刷新或重新打开后恢复上次选中的运行、诊断抽屉开关状态、当前诊断 tab、按 run 记住的 Trace/活动页选中消息、侧栏搜索/状态/inbox 筛选，以及按 run 隔离的新任务/运行 composer 草稿。顶部刷新按钮在选中 run 时会一起刷新该 run 的 status、context、provider trace、diff 和产物预览，而不只是刷新侧栏列表。它也会保留轻量、有界的本地对话 transcript：最近的选中 run 本地备注、压缩后的本地 Agent 回复，以及最新的新任务 Agent 回复都会跨刷新/重开恢复，但会刻意丢弃体积大的 `context`、`status`、`runs`、`diff` 和后台 job payload。如果保存的运行已经不存在，会自动回退到当前 inbox 焦点运行，而不是请求一个过期 run；如果当前 run inbox 不再包含保存的分组，过期 inbox 筛选会自动清除。草稿在成功提交后清除；选中 run 的自由文本发送失败时会保留输入。

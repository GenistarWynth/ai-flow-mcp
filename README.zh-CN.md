...
`context`、`handoff context`、`events`、`poll context` 和 `poll events` 是只读 Agent 消息：带 `run_id` 时直接返回当前运行的 Overview/Trace 诊断动作；不带 `run_id` 且存在最近运行时返回最新运行的 handoff 视图，不会推进阶段或绕过门禁。非 JSON 的 `patchbay agent message runs` 和 `patchbay agent message status` 会复用本地 inbox 文本视图，显示摘要、分组、焦点 run 和安全动作，而不是把完整结构化 payload 直接打印出来；非 JSON 的 `patchbay status`、`patchbay context` 和带 run_id 的对应 Agent 消息会显示 run 状态、门禁状态、后台任务摘要、失败恢复、Agent 活动、健康卡片、路由/效率证据、metrics、provider trail、产物和分组安全动作；活跃后台任务会在文本输出中显示 `Background polling` 和 `Background control` 分组，直接暴露轮询与取消控制，不需要用户查看 `JOB.json`；非 JSON 的 `patchbay events`、`patchbay trace`、`patchbay artifact` 和 Agent 诊断视图回复会显示时间线、产物预览、请求的诊断 tab、恢复提示和分组安全动作；非 JSON 的 `patchbay setup` / `install`、`patchbay doctor`、`patchbay config profile show/apply`、`patchbay metrics` 以及对应 Agent 消息也会显示 setup 步骤、就绪检查、write/fix 经济路由、效率证据、建议和分组安全动作；非 JSON 的 `help`、missing-run 交接、no-MCP/本地模式、`next step`、门禁状态和后台取消类 Agent 回复会显示能力、选中/最近 run、门禁诊断、下一步、取消结果和分组安全动作，而不是裸露结构化 payload；`--json` 仍保留机器可读契约。活跃后台任务会把 `poll_context`、`poll_status`、`poll_events` 和 `cancel_background_job` 提升到 `patchbay_context.next_actions`、顶层 `action_groups[]` 和 `agent_activity.conversation_state.suggestions`，桌面端/MCP host/Skill 可以直接用这些安全建议刷新或停止后台工作，不需要解析 `JOB.json`，也不会把 `continue`、`approve` 或 `apply` 暴露成后台直达动作。

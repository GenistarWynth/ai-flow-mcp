...
- CLI 流程：`setup`/`install`、`doctor`、`agent message`、`web`、`plan`、`approve`、`write`、`test`、`review`、`fix`、`status`、`context`、`metrics`、`events`、`trace`、`artifact`、`diff`、`apply`、`cleanup`。
...
`context`、`handoff context`、`events`、`poll context` 和 `poll events` 是只读 Agent 消息：带 `run_id` 时直接返回当前运行的 Overview/Trace 诊断动作；不带 `run_id` 且存在最近运行时返回最新运行的 handoff 视图，不会推进阶段或绕过门禁。非 JSON 的 `patchbay agent message runs` 和 `patchbay agent message status` 会复用本地 inbox 文本视图，显示摘要、分组、焦点 run 和安全动作，而不是把完整结构化 payload 直接打印出来；非 JSON 的 `patchbay status`、`patchbay context` 和带 run_id 的对应 Agent 消息会显示 run 状态、门禁状态、后台任务摘要、失败恢复、Agent 活动、健康卡片、路由/效率证据、metrics、provider trail、产物和分组安全动作；活跃后台任务会在文本输出中显示 `Background polling` 和 `Background control` 分组，直接暴露轮询与取消控制，不需要用户查看 `JOB.json`；无 run_id 的非 JSON `background status` / `后台状态` 会渲染最新活跃后台 job 摘要和同一组轮询/取消控制；非 JSON 的 `patchbay events`、`patchbay trace`、`patchbay artifact` 和 Agent 诊断视图回复会显示时间线、产物预览、请求的诊断 tab、恢复提示和分组安全动作；非 JSON 的 `patchbay setup` / `install`、`patchbay doctor`、`patchbay config profile show/apply`、`patchbay metrics` 以及对应 Agent 消息也会显示 setup 步骤、就绪检查、write/fix 经济路由、效率证据、建议和分组安全动作；非 JSON 的 `help`、missing-run 交接、no-MCP/本地模式、`next step`、门禁状态、后台状态和后台取消类 Agent 回复会显示能力、选中/最近 run、门禁诊断、下一步、后台状态、取消结果和分组安全动作，而不是裸露结构化 payload；`--json` 仍保留机器可读契约。活跃后台任务会把 `poll_context`、`poll_status`、`poll_events` 和 `cancel_background_job` 提升到 `patchbay_context.next_actions`、顶层 `action_groups[]` 和 `agent_activity.conversation_state.suggestions`，桌面端/MCP host/Skill 可以直接用这些安全建议刷新或停止后台工作，不需要解析 `JOB.json`，也不会把 `continue`、`approve` 或 `apply` 暴露成后台直达动作。

后台取消类文本视图也会在失败时渲染 `cancel_result` 的尝试、终止和错误信息，而不是要求用户检查原始结构化响应。
...
Web workbench 使用同一套对话式流程，并在诊断抽屉里提供“就绪”页。该页面调用统一 doctor 检查但默认不做 MCP stdio 探测，因此可以在桌面 UI 中看到安装与配置缺口，同时避免打开页面时额外启动子进程。就绪页的 MCP host 选择器会把目标 host 传给 doctor/setup，记住上次选中的 setup/readiness host，并把 `Claude Desktop`、`claude desktop`、`claude-desktop` 这类常见写法统一规范化为具体注册命令。该页面还会显示当前 write/fix 路由画像，并按 `action_groups[]` 把结构化 `actions[]` 分成稳定分区，用来执行安全的 setup、Skill、MCP 探测、刷新、经济路由和 Reasonix 命令配置后续操作。没有选中运行时，composer 上方会先展示紧凑“启动上下文”，汇总本地/no-MCP 模式、就绪状态、write/fix 经济路由和安全 setup/economy 动作，再创建新的计划 run。运行 composer 也会用 `patchbay_context.action_groups[]` 分组 `agent_activity.conversation_state.suggestions`，让后台轮询、后台控制、诊断和门禁动作分成独立控制区，而不是平铺按钮；后台任务卡片也会直接展示打开活动、刷新和取消控制。如果已安装的 Codex Skill 过期，就绪检查卡会直接显示 `status: outdated`、缺失/变更/多余的安装文件，以及安全的更新动作。本地对话回复会把安全的 `command` action 渲染成带复制兜底的命令行；当 Clipboard API 和兜底复制都失败时，会显示不可用状态，因此 MCP/Skill 注册命令和低成本 provider 配置命令的复制结果更清楚。选中运行的对话线程会先展示紧凑“运行概览”，汇总阶段/状态、门禁进度、经济路由健康和最近 provider 证据，再展示详细事件流。侧栏 run inbox 也会为每个 run 展示阶段、门禁、经济路由和最近 provider 的紧凑快速信号，方便打开详情前先分拣多任务队列。Overview 会展示 `efficiency_summary`、`routing_evidence.economy_health` 与 `agent_activity.health_cards`，包括 `economy_efficiency` 和 `economy_load` 卡片，让桌面端直接看到经济路由是健康、待观测、`command_not_ready`、漂移，还是未配置，并看到简单 write/fix 工作的实际 token/cost/time 占比。Trace/活动页会先把选中消息、运行时间线和 provider trace 渲染成可读卡片，原始 JSON 只保留在折叠明细中供精确排查。Diff/差异页会先按文件汇总变更、增删行和 hunk 数，原始 patch 文本保留在折叠明细中。Log/日志页和 Artifacts/产物页会复用 `failure_recovery`、状态错误、优先产物和当前产物预览，先展示失败摘要、建议下一步、错误线索和产物索引，再保留原始预览。Config/配置页会先摘要阶段路由、provider 命令、测试 allowlist、自定义 provider 和工作流安全开关，再把原始配置 JSON 放在折叠明细里。Providers/提供方页会汇总已配置阶段路由、实际 provider usage、事件轨迹、economy 目标、覆盖率和健康状态，让 write/fix 是否真的走低价模型一眼可查。

命令行控件会在复制按钮前直接展示原始命令文本，因此 provider 修复命令在截图里和剪贴板不可用时仍然可检查。
...
压缩后的本地 Agent 回复会在存在 `background_job` 时保留轻量摘要，因此刷新或重开后仍能看到已取消或失败的后台结果，同时不会持久化完整 handoff payload。

...
Workbench 还会在刷新或重新打开后恢复上次选中的运行、诊断抽屉开关状态、当前诊断 tab，以及按 run 隔离的新任务/运行 composer 草稿。如果保存的运行已经不存在，会自动回退到当前 inbox 焦点运行，而不是请求一个过期 run。草稿在成功提交后清除；选中 run 的自由文本发送失败时会保留输入。

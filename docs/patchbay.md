...
Workbench 会在刷新或重新打开后恢复最后选中的 run、诊断抽屉开关状态、当前诊断 tab、侧栏搜索/状态/inbox 筛选，以及按 run 隔离的新任务/运行 composer 草稿；如果保存的 run 不在当前列表中，会回退到 `runs.inbox.focus_run_id` 或列表首项，避免请求已经不存在的 run；如果当前 run inbox 不再包含保存的分组，过期 inbox 筛选会自动清除。草稿在成功提交后清除；选中 run 的自由文本发送失败时会保留输入。

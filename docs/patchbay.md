...
本地非 JSON `scripts/patchbay doctor` 和 `scripts/patchbay agent message readiness` 会把同一份 doctor/readiness payload 渲染为就绪状态、检查项、路由摘要、建议和按 `action_groups[]` 分组的安全动作，避免 no-MCP/Skill-only 路径把嵌套 JSON 直接暴露给用户；加 `--json` 时仍返回完整结构化契约。

...
`patchbay_runs`、`scripts/patchbay runs --json` 和无 `run_id` 的 Agent `status` / `runs` 响应会返回同一份结构化 Agent inbox。顶层 `runs.inbox` 包含 `groups`、`focus_run_id`、`active_count`、`confirmation_required_count` 和摘要；每个 run 包含 `current_phase`、`next_commands`、`gate_state`、`background_job`、`inbox`、`actions[]` 和 `action_groups[]`。`inbox.key` 会把 run 归为 `running`、`needs_approval`、`ready_to_apply`、`failed`、`ready_to_continue`、`inspect` 或 `applied`，桌面/MCP/Skill 客户端可直接把它渲染成多任务工作队列。`inbox.next_action` 中的 `approve_and_run` 与 `apply` 始终带 `requires_confirmation` 且 `safe: false`；只读 `open_run`、诊断 tab、后台轮询和失败检查动作才应作为直接安全按钮。
